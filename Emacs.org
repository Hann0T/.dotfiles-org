...
(keymap-set hann0t-map "s p" #'surround-region-w-parentheses)
(keymap-set hann0t-map "s t" #'surround-region-w-tilde)
(keymap-set hann0t-map "s d" #'surround-region-w-double-quote)
(keymap-set hann0t-map "e r" #'code/focus-region)
(keymap-set hann0t-map "e k" #'kill-buffer-and-window)
...
;; Tell Emacs to remap old modes to treesit ones
...
* Edit Indirect
#+begin_src emacs-lisp
(defun code/focus-region ()
  (interactive)
  (let ((start (region-beginning))
        (end (region-end)))
    (clone-indirect-buffer-other-window "*focus*" t)
...
(use-package expenses
  :ensure t)
(setq expenses-directory "/home/hann0t/Documents/expenses")
(setq expenses-category-list '("Clothes" "Electronics" "Entertainment" "Fee" "Food" "Gift" "Health" "Home" "Other" "Subscriptions" "Transfer" "Utilities" "Education" "Hobbies"))
(setq expenses-currency "S/")
#+end_src
* Functions
#+begin_src emacs-lisp
