...
(define-prefix-command 'hann0t-window-map)
(global-set-key (kbd "C-w") 'hann0t-window-map)
(keymap-set hann0t-window-map "o" #'other-window)
(keymap-set hann0t-window-map "q" #'delete-window)
(keymap-set hann0t-window-map "O" #'delete-other-windows)
(keymap-set hann0t-window-map "v" #'split-window-right)
(keymap-set hann0t-window-map "h" #'split-window-below)
...
** Harponee
#+begin_src emacs-lisp
;;(defun harponee/add-file ()
  ;;(interactive)
  ;;(let ((name (format "file-%d" (length bookmark-alist))))
  ;;(bookmark-set name))
;;)
#+end_src
** Workflow automation
